"""
ocr_adahessian_64.py
====================
MNIST OCR — OCRConvNetTriple, AdaHessian optimizer, 64×64
...
Output: E:\\CSC-114\\project\\pytorch_adahessian_64\\
"""
...
DROP_PATH     = 0.05
DROPOUT_HEAD  = 0.35
OUTPUT_DIR    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pytorch_adahessian_64")
...
class _Tee:
    ...
    Log location: <OUTPUT_DIR>/adahessian_64_<YYYYMMDD_HHMMSS>.log
    """
    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = os.path.join(log_dir, f"adahessian_64_{ts}.log")
...
def plot_curves(log_path, out_path):
    ...
    ax.set_title("OCRConvNetTriple — AdaHessian 64x64")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  OCRConvNetTriple — AdaHessian — 64x64")
    ...
    log_path    = os.path.join(OUTPUT_DIR, "adahessian_64_training_log.csv")
    resume_path = os.path.join(OUTPUT_DIR, "adahessian_64_resume.json")
    best_path   = os.path.join(OUTPUT_DIR, "adahessian_64_best.pt")
    final_path  = os.path.join(OUTPUT_DIR, "adahessian_64_final.pt")
    onnx_path   = os.path.join(OUTPUT_DIR, "adahessian_64.onnx")
    curve_path  = os.path.join(OUTPUT_DIR, "adahessian_64_curves.png")
