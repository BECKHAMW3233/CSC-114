...
class _Tee:
    ...
    appear on stdout.
    Log location: <OUTPUT_DIR>/soap_128_<YYYYMMDD_HHMMSS>.log
    """
    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = os.path.join(log_dir, f"soap_128_{ts}.log")
...
def plot_curves(log_path, out_path):
    ...
    ax.set_title("OCRConvNetTriple — SOAP 128x128")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  OCRConvNetTriple — SOAP — 128x128")
    ...
    log_path    = os.path.join(OUTPUT_DIR, "soap_128_training_log.csv")
    resume_path = os.path.join(OUTPUT_DIR, "soap_128_resume.json")
    best_path   = os.path.join(OUTPUT_DIR, "soap_128_best.pt")
    final_path  = os.path.join(OUTPUT_DIR, "soap_128_final.pt")
    onnx_path   = os.path.join(OUTPUT_DIR, "soap_128.onnx")
    curve_path  = os.path.join(OUTPUT_DIR, "soap_128_curves.png")
