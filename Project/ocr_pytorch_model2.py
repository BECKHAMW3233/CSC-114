...
=====================
MNIST OCR — Model 2 for Ensemble
...
OUTPUT_ROOT      = Path(r"E:\CSC-114\emnist-model\pytorch2")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v4_model2_best_model_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v4_model2_final_model_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v4_model2_ocr_mod_{img_size}.onnx"),
        "log_path":         str(OUTPUT_ROOT / f"v4_model2_training_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v4_model2_training_curves_{img_size}.png"),
    }
...
def plot_history(history: dict, path: str, img_size: int):
    ...
    fig.suptitle(f"MNIST OCR Model 2 (Schedule-Free AdamW) {img_size}x{img_size} — Training History",
    ...
    with open(path, "w", newline="") as f:
        ...
        for i in range(n):
            writer.writerow({
                ...
                "vram_alloc_gb":    history.get("vram_alloc_gb",   ["-1"]*n)[i],
                "vram_reserved_gb": history.get("vram_reserved_gb",["-1"]*n)[i],
...
def run_training(img_size: int):
    ...
    print("=" * 60)
    print(f"  MNIST OCR — Model 2 (v4 — Schedule-Free AdamW)  [{img_size}x{img_size}]")
    ...
    # Optional ensemble check against Model 1 at the SAME resolution, if available
    model1_path = Path(r"E:\CSC-114\emnist-model\pytorch") / f"v4_model1_best_model_{img_size}.pt"
    if model1_path.exists():
        try:
            ...
            print(f"\n{'='*40}")
            print(f"  [{img_size}x{img_size}] Model 1 alone : see pytorch/v4_model1_training_log_{img_size}.csv")
            print(f"  [{img_size}x{img_size}] Model 2 alone : {test_acc*100:.2f}%")
            print(f"  [{img_size}x{img_size}] ENSEMBLE M1+M2: {ensemble_acc*100:.2f}%")
            print(f"{'='*40}")
        except Exception as e:
            print(f"[Ensemble] Could not load Model 1 at {img_size}x{img_size}: {e}")
    else:
        print(f"\n[Ensemble] Model 1 checkpoint for {img_size}x{img_size} not found yet — skipping ensemble check.")
...
def main():
    print("\n" + "#" * 60)
    print(f"  MNIST MODEL 2 — DUAL-RESOLUTION SEQUENTIAL TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  ALL RESOLUTIONS COMPLETE — MODEL 2")
...
if __name__ == "__main__":
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / "v4_model2_training_lCLI_OUTPUT_ALL.txt")
    main()
