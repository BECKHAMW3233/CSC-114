...
OUTPUT_ROOT      = Path(r"E:\CSC-114\project\lion_64")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v1_lion_64_best_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v1_lion_64_final_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v1_lion_64_{img_size}.onnx"),
        "quantized_path":   str(OUTPUT_ROOT / f"v1_lion_64_quantized_{img_size}.pt"),
        "log_path":         str(OUTPUT_ROOT / f"v1_lion_64_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v1_lion_64_curves_{img_size}.png"),
        "resume_path":      str(OUTPUT_ROOT / f"v1_lion_64_resume_{img_size}.json"),
    }
...
def main(batch_override=None):
    print("\n" + "#" * 60)
    print(f"  MNIST Lion 64x64 — SINGLE RESOLUTION TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — LION 64x64")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v1_lion_64_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
