...
OUTPUT_ROOT      = Path(r"E:\CSC-114\project\adamw_128")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v1_adamw_128_best_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v1_adamw_128_final_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v1_adamw_128_{img_size}.onnx"),
        "log_path":         str(OUTPUT_ROOT / f"v1_adamw_128_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v1_adamw_128_curves_{img_size}.png"),
        "resume_path":      str(OUTPUT_ROOT / f"v1_adamw_128_resume_{img_size}.json"),
    }
...
def main(batch_override=None):
    print("\n" + "#" * 60)
    print(f"  MNIST ScheduleFree-AdamW 128x128 — SINGLE RESOLUTION TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — SCHEDULEFREE-ADAMW 128x128")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v1_adamw_128_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
