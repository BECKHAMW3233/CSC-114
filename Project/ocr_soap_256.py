...
ocr_soap_256.py
===============
MNIST OCR — OCRConvNetTriple, SOAP optimizer, 256×256
...
OPTIMIZER — SOAP (Shampoo + Adam, Kronecker-factored second-order):
  ...
  Run with: python ocr_soap_256.py --batch-size 128
...
Hardware target:
    ...
    64 GB DDR5-5600
    RTX 4080 16 GB  — batch TBD (viability not yet confirmed at 256×256)

Output: E:\\CSC-114\\project\\pytorch_soap_256\\
"""
...
DROP_PATH     = 0.05
DROPOUT_HEAD  = 0.35
OUTPUT_DIR    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pytorch_soap_256")
...
class _Tee:
    ...
    appear on stdout.
    Log location: <OUTPUT_DIR>/soap_256_<YYYYMMDD_HHMMSS>.log
    """
    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = os.path.join(log_dir, f"soap_256_{ts}.log")
    ...
    for bs in [512, 256, 128, 64]:
        try:
            ...
            if "out of memory" in _emsg or "find was unable" in _emsg or "engine" in _emsg:
                ...
                raise
    raise RuntimeError("OOM or shared VRAM spillover at all batch sizes — cannot continue")
...
def plot_curves(log_path, out_path):
    ...
    ax.set_title("MNIST OCR — OCRConvNetTriple — SOAP 256x256")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  MNIST OCR — OCRConvNetTriple — SOAP — 256x256")
    ...
    log_path    = os.path.join(OUTPUT_DIR, "soap_256_training_log.csv")
    resume_path = os.path.join(OUTPUT_DIR, "soap_256_resume.json")
    best_path   = os.path.join(OUTPUT_DIR, "soap_256_best.pt")
    final_path  = os.path.join(OUTPUT_DIR, "soap_256_final.pt")
    onnx_path   = os.path.join(OUTPUT_DIR, "soap_256.onnx")
    curve_path  = os.path.join(OUTPUT_DIR, "soap_256_curves.png")
