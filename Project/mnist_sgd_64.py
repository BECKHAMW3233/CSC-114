...
mnist_sgd_64.py
===============
Digit OCR — OCRConvNetTriple, SGD + Nesterov, 64×64
...
Watch point — non-monotonic resolution behavior:
  ...
  directly tests whether that inversion is a class-count artifact (this run
  uses 10 digit classes, not EMNIST's 62) or an SGD fundamental behavior.
  Per-class accuracy at each resolution will be compared against the v4 baseline.

Hardware (confirmed 2026-07-07):
    AMD Ryzen 9 7900X  (24 threads)
    64 GB DDR5-5600
    RTX 4080 16 GB  — batch=512 override, 11.9/15.1GB VRAM peak, 57–60°C
    Completed: 99.49% test accuracy, 104 epochs, stopped by patience (20)
...
Running this script:
  ...
  Force a specific batch size (skips auto-detect entirely):
    python mnist_sgd_64.py --batch-size 512

  Resumable: if v1_sgd_64_resume_64.pt and v1_sgd_64_best_64.pt both
...
Output: E:\\CSC-114\\project\\sgd_64\\
"""
...
OUTPUT_ROOT      = Path(r"E:\CSC-114\project\sgd_64")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v1_sgd_64_best_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v1_sgd_64_final_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v1_sgd_64_{img_size}.onnx"),
        "log_path":         str(OUTPUT_ROOT / f"v1_sgd_64_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v1_sgd_64_curves_{img_size}.png"),
        "resume_path":      str(OUTPUT_ROOT / f"v1_sgd_64_resume_{img_size}.json"),
    }
...
def main(batch_override=None):
    print("\n" + "#" * 60)
    print(f"  MNIST SGD 64x64 — SINGLE RESOLUTION TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — SGD 64x64")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v1_sgd_64_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
