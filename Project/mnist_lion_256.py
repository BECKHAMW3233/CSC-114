...
mnist_lion_256.py
=================
MNIST OCR — OCRConvNet, Lion optimizer, 256×256
...
Architecture — OCRConvNet:
  ...
  Depthwise-separable convolutions (spatial + cross-channel split)
  Residual connections, BatchNorm, GlobalAveragePooling
  Classifier head: 256→128→10
  ~2.5M parameters, ~9.4 MB (float32)

Book references — Chollet & Watson, "Deep Learning with Python, 3rd Ed." (Manning 2025)
  Ch. 2  — Tensors, matrix multiply, gradient descent, backpropagation, loss functions
  Ch. 3  — PyTorch nn.Module, backward(), optimizer.step(), zero_grad()
...
OPTIMIZER — Lion (Evolved Sign Momentum):
  ...
  Converges to smoother loss minima than Adam — better real-world generalization.
  Requires: pip install lion-pytorch
  Key properties:
    - LR ~10x lower than Adam (3e-5)
    - Weight decay ~10x higher than Adam (1e-2)
    - Memory efficient: one momentum buffer vs Adam's two
    - Betas (0.9, 0.99) per paper recommendation for image classification
...
Hardware (confirmed 2026-07-06):
    AMD Ryzen 9 7900X  (24 threads)
    64 GB DDR5-5600
    RTX 4080 16 GB  — batch 128 override required, 15.6/16.0GB dedicated,
                       1.0GB shared spillover, ~15-20min/epoch est.
    Run with: python mnist_lion_256.py --batch-size 128

Output: E:\\CSC-114\\project\\lion_256\\
"""
...
OUTPUT_ROOT      = Path(r"E:\CSC-114\project\lion_256")
...
def determine_batch_size(model_cls, img_size: int, device: torch.device,
    ...
    """
    print(f"[Batch] Auto-detecting batch size for {img_size}x{img_size} (256x256 safe range: 128/64/32)...")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v1_lion_256_best_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v1_lion_256_final_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v1_lion_256_{img_size}.onnx"),
        "quantized_path":   str(OUTPUT_ROOT / f"v1_lion_256_quantized_{img_size}.pt"),
        "log_path":         str(OUTPUT_ROOT / f"v1_lion_256_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v1_lion_256_curves_{img_size}.png"),
        "resume_path":      str(OUTPUT_ROOT / f"v1_lion_256_resume_{img_size}.json"),
    }
...
def plot_history(history: dict, path: str, img_size: int):
    ...
    fig.suptitle(f"MNIST OCR Lion {img_size}x{img_size} — Training History",
    ...
    with open(path, "w", newline="") as f:
        ...
        for i in range(len(history["train_loss"])):
            writer.writerow({
                ...
                "vram_peak_alloc_gb":   history.get("vram_peak_alloc_gb",   ["-1"] * (i+1))[i],
                "vram_peak_reserved_gb":history.get("vram_peak_reserved_gb",["-1"] * (i+1))[i],
...
def run_training(img_size: int, batch_override: int = None):
    ...
    print("=" * 60)
    print(f"  MNIST OCR — Lion  [{img_size}x{img_size}]")
...
def main(batch_override=None):
    print("\n" + "#" * 60)
    print(f"  MNIST Lion 256x256 — SINGLE RESOLUTION TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — LION 256x256")
...
if __name__ == "__main__":
    ...
    _parser.add_argument('--batch-size', type=int, default=None,
                         help='Override auto batch detection')
    _args = _parser.parse_args()
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v1_lion_256_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
