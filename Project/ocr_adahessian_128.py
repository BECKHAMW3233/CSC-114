"""
ocr_adahessian_128.py
=====================
MNIST OCR — OCRConvNetTriple, AdaHessian optimizer, 128×128
...
OPTIMIZER — AdaHessian (second-order, diagonal Hessian approximation):
  lr=0.15, betas=(0.9, 0.999), eps=1e-4, weight_decay=5e-4, hessian_power=1.0
  500-step linear warmup before CosineAnnealingLR
  PATIENCE=20
  Critical requirements:
    - loss.backward(create_graph=True) required for Hessian computation
    - p.grad must be set to None after optimizer.step() to break reference
      cycle and prevent cumulative VRAM leak
    - AMP disabled — create_graph=True requires float32 gradient graph
    - 256×256 not supported — create_graph=True roughly doubles VRAM of
      backward pass, infeasible even at minimum batch size on 16GB VRAM
...
Output: E:\\CSC-114\\project\\pytorch_adahessian_128\\
"""
...
DROP_PATH     = 0.05
DROPOUT_HEAD  = 0.35
OUTPUT_DIR    = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pytorch_adahessian_128")
...
class _Tee:
    ...
    Capturing stderr is required to catch PyTorch UserWarnings (e.g. the
    create_graph reference cycle warning) that do not appear on stdout.
    Log location: <OUTPUT_DIR>/adahessian_128_<YYYYMMDD_HHMMSS>.log
    """
    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        ts       = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = os.path.join(log_dir, f"adahessian_128_{ts}.log")
...
def try_batch_size(img_size):
    """
    Probe largest safe batch size using create_graph=True backward to accurately
    reflect AdaHessian VRAM requirements. Nulls .grad after backward to break
    the reference cycle before cleaning up the probe model.
    Candidates: 512 -> 256 -> 128 -> 64
    128x128 with AdaHessian expected to land at 128 or 64.
    """
    for bs in [512, 256, 128, 64]:
        try:
            ...
            # Break reference cycle before cleanup
...
def train_epoch(model, loader, optimizer, scheduler, criterion):
    ...
    for imgs, labels in loader:
        ...
        # AdaHessian requires create_graph=True for Hessian diagonal approximation.
        # No AMP — full precision required.
        loss.backward(create_graph=True)
        optimizer.step()
        # CRITICAL: null .grad on every parameter after step to break the reference
        # cycle created by create_graph=True. Without this, VRAM leaks continuously
        # across steps until OOM. (PyTorch UserWarning, engine.cpp:1206)
        for p in model.parameters():
            p.grad = None
        scheduler.step()
...
def plot_curves(log_path, out_path):
    ...
    ax.set_title("OCRConvNetTriple — AdaHessian 128x128")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  OCRConvNetTriple — AdaHessian — 128x128")
    ...
    log_path    = os.path.join(OUTPUT_DIR, "adahessian_128_training_log.csv")
    resume_path = os.path.join(OUTPUT_DIR, "adahessian_128_resume.json")
    best_path   = os.path.join(OUTPUT_DIR, "adahessian_128_best.pt")
    final_path  = os.path.join(OUTPUT_DIR, "adahessian_128_final.pt")
    onnx_path   = os.path.join(OUTPUT_DIR, "adahessian_128.onnx")
    curve_path  = os.path.join(OUTPUT_DIR, "adahessian_128_curves.png")
