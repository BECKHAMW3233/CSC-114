...
=====================
MNIST OCR — Model 3 for Ensemble (Maximum Capacity)
...
CORRECTIONS APPLIED (v2):
  ...
  - WeightedRandomSampler added — addresses MNIST class imbalance
...
OUTPUT_ROOT      = Path(r"E:\CSC-114\emnist-model\pytorch3")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v4_model3_best_model_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v4_model3_final_model_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v4_model3_ocr_mod_{img_size}.onnx"),
        "log_path":         str(OUTPUT_ROOT / f"v4_model3_training_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v4_model3_training_curves_{img_size}.png"),
    }
...
def run_training(img_size: int):
    ...
    print("=" * 60)
    print(f"  MNIST OCR — Model 3 (v4 — Triple + MultiScale)  [{img_size}x{img_size}]")
    ...
    print(f"\n[Ensemble] [{img_size}x{img_size}] Looking for Models 1 and 2 at the same resolution...")
    try:
        import sys
        ...
        m1_path = Path(r"E:\CSC-114\emnist-model\pytorch")  / f"v4_model1_best_model_{img_size}.pt"
        m2_path = Path(r"E:\CSC-114\emnist-model\pytorch2") / f"v4_model2_best_model_{img_size}.pt"

        if not m1_path.exists() or not m2_path.exists():
            print(f"[Ensemble] Model 1 and/or Model 2 checkpoint for {img_size}x{img_size} "
                  f"not found yet — skipping ensemble check.")
        else:
            ...
            print(f"\n{'='*55}")
            print(f"  [{img_size}x{img_size}] FINAL ENSEMBLE RESULTS")
            print(f"  ─────────────────────────────────────────────────")
...
def main():
    print("\n" + "#" * 60)
    print(f"  MNIST MODEL 3 — DUAL-RESOLUTION SEQUENTIAL TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  ALL RESOLUTIONS COMPLETE — MODEL 3")
...
if __name__ == "__main__":
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / "v4_model3_training_lCLI_OUTPUT_ALL.txt")
    main()
