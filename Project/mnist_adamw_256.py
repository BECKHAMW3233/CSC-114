...
mnist_adamw_256.py
==================
MNIST OCR — OCRConvNetWide, Schedule-Free AdamW, 256×256
Pure PyTorch — Wider architecture with Squeeze-Excitation attention blocks.

*** HARDWARE INFEASIBLE ON 16GB VRAM ***
Confirmed 2026-07-06: OCRConvNetWide at 256×256, batch 128 produced 5.5GB
shared GPU memory spillover and 21.0GB total GPU memory usage. The 512-channel
stage 4 exceeds 16GB dedicated VRAM at this resolution at any practical batch
size. Do not run on RTX 4080 16GB or equivalent. Retained in the repository
as a documented attempt.
...
Hardware (confirmed infeasible):
    RTX 4080 16 GB — 21.0GB total GPU memory at batch 128, 5.5GB shared spillover

Output: E:\\CSC-114\\project\\adamw_256\\
"""
...
OUTPUT_ROOT      = Path(r"E:\CSC-114\project\adamw_256")
...
def build_config(img_size: int, batch_size: int) -> dict:
    ...
    return {
        ...
        "checkpoint_path":  str(OUTPUT_ROOT / f"v1_adamw_256_best_{img_size}.pt"),
        "final_model_path": str(OUTPUT_ROOT / f"v1_adamw_256_final_{img_size}.pt"),
        "onnx_path":        str(OUTPUT_ROOT / f"v1_adamw_256_{img_size}.onnx"),
        "log_path":         str(OUTPUT_ROOT / f"v1_adamw_256_log_{img_size}.csv"),
        "plot_path":        str(OUTPUT_ROOT / f"v1_adamw_256_curves_{img_size}.png"),
        "resume_path":      str(OUTPUT_ROOT / f"v1_adamw_256_resume_{img_size}.json"),
    }
...
def main(batch_override=None):
    print("\n" + "#" * 60)
    print(f"  MNIST ScheduleFree-AdamW 256x256 — SINGLE RESOLUTION TRAINING")
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — SCHEDULEFREE-ADAMW 256x256")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v1_adamw_256_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
