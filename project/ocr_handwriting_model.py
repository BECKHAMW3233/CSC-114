...
# =============================================================================
# 2. DATASET — EMNIST loaded fully into RAM as numpy arrays (Ch. 2, Ch. 8)
# =============================================================================
# Strategy: load entire dataset into RAM once, pass as numpy arrays to Keras.
# With 64 GB DDR5 the full EMNIST byclass (~560 MB) fits easily.
# This eliminates the generator/DataLoader bottleneck that causes ~38x slowdown
# when feeding Keras on the PyTorch backend via a Python generator.
# Keras receives a numpy array → hands it directly to the PyTorch backend →
# GPU runs at full utilization, matching pure PyTorch training speed.
#
# Augmentation: applied via Keras augmentation layers in the model itself
# (build_augmentation_layer), which run on GPU during training.
# torchvision CPU augmentation is skipped — not needed when Keras does it.

def load_emnist_numpy(data_dir: Path):
    """
    Downloads EMNIST byclass via torchvision, then converts the full dataset
    to numpy arrays in (N, H, W, C) format for direct Keras consumption.

    Ch. 2: normalize pixel values to float32 [0, 1] → then to [-1, 1].
    Ch. 8: channels-last (H, W, C) is the Keras/TF default data format.

    Returns:
        x_train, y_train  — float32 (N, 32, 32, 1), int64 labels
        x_val,   y_val
        x_test,  y_test
    """
    ...
    # Load raw datasets (no augmentation transforms — Keras handles that on GPU)
    ...
    print(f"[Dataset] Converting to numpy arrays (this takes ~60s)...")

    def to_numpy(dataset, indices=None):
        """
        Bulk-load dataset into numpy arrays using a fast DataLoader.
        Uses num_workers=8 for parallel loading, then stacks into one array.
        permute (N,C,H,W) → (N,H,W,C) for Keras channels-last convention.
        """
        ...
        xs, ys = [], []
        for x, y in loader:
            # (N, C, H, W) → (N, H, W, C) then to numpy
            xs.append(x.permute(0, 2, 3, 1).numpy())
            ys.append(y.numpy())
        return np.concatenate(xs, axis=0), np.concatenate(ys, axis=0)

    x_train, y_train = to_numpy(train_full, train_idx.indices)
    x_val,   y_val   = to_numpy(train_full, val_idx.indices)
    x_test,  y_test  = to_numpy(test_ds)

    print(f"[Dataset] Train: {len(x_train):,} | Val: {len(x_val):,} | "
          f"Test: {len(x_test):,}")
    print(f"[Dataset] x_train shape: {x_train.shape}  dtype: {x_train.dtype}")
...
def train_model(model:       keras.Model,
    ...
    callbacks = [
        ...
        # TensorBoard requires tensorflow — omitted in pure PyTorch backend venv
    ]
...
def evaluate_model(model: keras.Model, test_data, test_labels=None):
...
def main():
    ...
    # Step 1 — Prepare data: load entire dataset into RAM as numpy arrays.
    # With 64 GB DDR5 this fits easily. Keras receives numpy → full GPU speed.
    (x_train, y_train), (x_val, y_val), (x_test, y_test) = load_emnist_numpy(DATA_DIR)
    ...
    # Pass numpy arrays directly — Keras handles batching internally at GPU speed
    print("\n[Phase 1] Training...")
