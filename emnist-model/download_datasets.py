...
# ── Model paths — home machine ─────────────────────────────────────────────────
MODELS = [
    # Base models
    r"E:\CSC-114\emnist-model\pytorch\ocr_model.onnx",
    r"E:\CSC-114\emnist-model\pytorch2\ocr_model2.onnx",
    r"E:\CSC-114\emnist-model\pytorch3\ocr_model3.onnx",
    # Distilled models
    r"E:\CSC-114\emnist-model\pytorch_distill1\ocr_model1_distill.onnx",
    r"E:\CSC-114\emnist-model\pytorch_distill2\ocr_model2_distill.onnx",
    r"E:\CSC-114\emnist-model\pytorch_distill3\ocr_model3_distill.onnx",
]
...
def apply_spatial_override(label, box_w, box_h, median_h, mode, all_top3=None):
    if box_h == 0:
        return label
    aspect = box_w / box_h
    height_ratio = box_h / median_h if median_h > 0 else 1.0
    if aspect < 0.30 and height_ratio > 0.7:
        if label in ("L", "l", "I", "T", "t", "J", "j", "Y", "y", "1"):
            if mode in ("digits", "digits-strict"):
                ...
                # In auto mode check if any model predicted i or I
                if all_top3:
                    i_score = sum(
                    ...
                    if i_score > 0.15:
                        return "i"
                return "1" if label in ("L", "I", "T", "Y", "1") else label
    return label
...
def vote_topn(all_top3, conf_threshold=0.20, mode="auto"):
    ...
    if top_count == n:
        ...
        if len(sorted_scores) > 1 and sorted_scores[0] > sorted_scores[1] * 1.3:
            # q is almost always a misread of a in auto mode — remap it
            if best == "q" and mode == "auto":
                return "a", "WEIGHTED", top1_labels
            return best, "WEIGHTED", top1_labels

        # Split rescue — if one model has a clearly dominant top-1 confidence
        # and no other model agrees on anything else, use it
        ...
        if best_conf > 0.22:
            ...
            if not other_confs or best_conf > max(other_confs) * 1.5:
...
def run_pipeline(image_path, sessions, img_sizes, model_names, mode="auto"):
    print(f"\n{'='*60}")
    print(f"  OCR Pipeline — 6-Model Ensemble (Base + Distilled)")
    ...
    for line in lines:
        ...
        for (x, y, w, h) in line:
            ...
            winner = apply_spatial_override(winner, w, h, median_h, mode, all_top3)
    ...
    if mode == "digits-strict":
        ensemble_lines, corrections, strict_applied, line_counts = apply_strict_grid(ensemble_lines)
    ...
    print(f"\n  Consensus: {agree_count}/{total_count} chars ({pct:.1f}% all-6 agreement)")
    ...
    # Character detail
    print(f"\n  {'─'*50}")
    print(f"  CHARACTER DETAIL  (M1 | M2 | M3 | M1d | M2d | M3d | vote | final)")
    print(f"  {'─'*50}")
    for ln, (line_boxes, ens_line) in enumerate(zip(lines, ensemble_lines), 1):
        ...
        for box_idx, (x, y, w, h) in enumerate(line_boxes):
            ...
            flag = ("✓ all"    if agreement == "ALL"      else
                    ...
                    "✗ split")
            aspect = w/h if h > 0 else 0
            cols = " | ".join(raw_top1s) if raw_top1s else "?"
            print(f"    Char {box_idx+1:>2} [asp:{aspect:.2f}]: {cols} | {flag} -> {final_label}")
            if agreement == "SPLIT":
                for mi, top3 in enumerate(all_top3):
                    top3_str = "/".join(f"{l}({c:.0%})" for l, c in top3[:3])
                    print(f"             M{mi+1}: {top3_str}")
            char_idx += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="OCR Pipeline — 6-Model Ensemble (Base + Distilled) with mode remapping",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
MODES:
  auto           Raw ensemble output, no remapping (default)
  digits         Remap letter predictions to digit equivalents
  digits-strict  Like digits, plus position-based correction for 0-9 grid
  upper          Force uppercase
  lower          Force lowercase

EXAMPLES:
  python home_test_full.py test1.jpg
  python home_test_full.py --mode digits test*.jpg
  python home_test_full.py --mode digits-strict test*.jpg
  python home_test_full.py --mode auto untitled.png
        """
    )
